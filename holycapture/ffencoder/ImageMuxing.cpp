...
static int write_frame(AVFormatContext *fmt_ctx, const AVRational *time_base, AVStream *st, AVPacket *pkt, OutputStream *ost)
{
	...
	//AVRational time_base = m_pCodeCon->time_base;
	//int64_t calc_duration = (double)AV_TIME_BASE / av_q2d(ost->enc->framerate);

	pkt->pts = av_rescale_q_rnd(pkt->pts, ost->enc->time_base, ost->enc->time_base, AV_ROUND_NEAR_INF);
	pkt->dts = av_rescale_q_rnd(pkt->dts, ost->enc->time_base, ost->enc->time_base, AV_ROUND_NEAR_INF);
	pkt->duration = av_rescale_q(pkt->duration, ost->enc->time_base, ost->enc->time_base);
...
static int write_video_frame(AVFormatContext *oc, OutputStream *ost, uint8_t* pRgb)
{
	...
	if (got_packet)
	{
		...
		ret = write_frame(oc, &c->time_base, ost->st, &pkt, ost);
